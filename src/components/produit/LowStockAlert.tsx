...
const LowStockAlert: React.FC<LowStockAlertProps> = ({ products }) => {
  ...
  return (
    <div className="bg-red-900/20 border border-red-700 rounded-lg p-4 md:p-6">
        ...
        {lowStockProducts.map(product => (
          <div key={product.id} className="bg-red-900/30 border border-red-600 rounded-lg p-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-red-400 font-bold text-lg">Stock: {product.stock}</p>
                <p className="text-gray-300 text-sm">{product.price.toFixed(2)}€</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-400">Valeur restante</p>
                <p className="text-white font-medium">{(product.price * product.stock).toFixed(2)}€</p>
              </div>
            </div>
            
            {product.sku && (
              <p className="text-gray-500 text-xs mt-2">Code: {product.sku}</p>
            )}
          </div>
        ))}
      </div>
